...
const parseIP = (req) => {
  let protocol = req.headers["X-Forwarded-For"] || req.headers["x-forwarded-for"] || req.client.remoteAddress
...
const randomColor = () => {
  let colors = ['red', 'blue', 'green', 'grey', 'orange', 'black', 'yellow', 'purple']
  let min = Math.ceil(0)
